...
# Landmarks
landmarks = [
    {"name": "USAO University", "lat": 35.0350, "lng": -97.9480, "color": "#00BFFF"},
...
# SURGICAL DATA HEATMAP - FIXING THE YELLOW MID-TONES
overlay = Image.new('RGBA', image.size, (0,0,0,0))
grid_res = 150 # Even higher grid fidelity
heatmap_data = np.zeros((grid_res, grid_res, 4))

# Intelligence Points
danger_pockets = [
    {"lat": 35.0415, "lng": -97.9343, "strength": 0.012, "weight": 1.0}, # 3rd St Strip
...
safe_anchors = [
    {"lat": 35.0340, "lng": -97.9580, "strength": 0.020, "weight": 1.0}, # S.West Elite
    {"lat": 35.0350, "lng": -97.9480, "strength": 0.012, "weight": 0.6}, # USAO Anchor
]
...
for r in range(grid_res):
    for c in range(grid_res):
        ...
        # 1. Start with a neutral Yellow (Transitional baseline)
        r_val, g_val, b_val = 255, 255, 0
        
        # 2. Apply Latitude Gradient (Fades yellow to red north, yellow to green south)
        lat_ratio = (cur_lat - bottom_lat) / (top_lat - bottom_lat) # 0 south, 1 north
        if lat_ratio > 0.6: # Fading to red in the north
            g_val = int(255 * (1 - (lat_ratio - 0.6) / 0.4))
        elif lat_ratio < 0.4: # Fading to green in the south
            r_val = int(255 * (lat_ratio / 0.4))
            
        # 3. Apply Danger Point Influence (Forces Red)
        for dp in danger_pockets:
            ...
            if dist < dp['strength']:
                ...
                g_val = int(g_val * (1-inf)) # Kill green to force red
        
        # 4. Apply Safe Anchor Influence (Forces Green)
        for sa in safe_anchors:
            ...
            if dist < sa['strength']:
                ...
                r_val = int(r_val * (1-inf)) # Kill red to force green
                
        heatmap_data[r, c] = [r_val, g_val, b_val, 80] # Opacity 80
...
def draw_pin(draw, x, y, color, label, is_landmark=False):
    ...
    if is_landmark:
        ...
        draw.rectangle([x+30, y-75, x+280, y-30], fill=(0,0,0,230), outline=color, width=2)
...
for r in roads:
    ...
    draw.text((px-85, py-10), r['name'], fill='#FFFF00') # Yellow text for visibility

# HUD
draw.rectangle([50, 50, 650, 350], fill=(0,0,0,250), outline='white', width=4)
draw.text((80, 70), "BRASH TACTICAL COMMAND - CHICKASHA V8 (REFINED)", fill='white')
draw.line([80, 100, 620, 100], fill='white', width=2)

y_pos = 120
legend = [
    ("#00FF00", "ELITE: High Owner-Occ / Top Schools"),
    ("#FFFF00", "TRANSITIONAL: The Sweet Spot / Rental Demand"),
    ("#FFA500", "SPECULATIVE: Hard Rehab / Block-by-Block"),
    ("#FF0000", "HIGH RISK: Industrial Blight / 3rd St Core"),
    ("#00BFFF", "LANDMARK: University / Industrial Anchors")
]
...
combined.save('/home/clawdbot/.openclaw/workspace/chickasha_tactical_v8_yellow.png')
print("V8 Yellow Refined Map Rendered.")
